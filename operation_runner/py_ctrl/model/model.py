...
def the_model() -> Model:
    ...
    initial_state = State(
        ...
        gripper_closed = False, #mine
        above_position = False, #mine
        robot_pose = "unknown",
        goal_as_string = "",
        replan = True,
    ...
    ops = {} #creates an empty dictionary

    # this is maybe the simplest operation, to make the robot robot to the buffer position
    ops[f"op_move_to_buffer"] = Operation(

        # the name of the variables and values must be unique and can not be the same as any other operation
    ...
    ops[f"op_move_to_pos_2"] = Operation(
        name = f"op_move_to_pos_2",
        precondition = Transition("post", 
                                  g(f"!robot_run && robot_state == initial && robot_pose != pose_2"), 
    ...
    ops[f"move_to_pos_3"] = Operation(
        name = f"move_to_pos_3",
        precondition = Transition("pre", 
                                  g(f"!robot_run && robot_state == initial && robot_pose != pose_3"), 
    ...
    for i in ["buffer", "pose_1", "pose_2","pose_3"]:
        ops[f"move_to_above_{i}"] = Operation(
            name = f"move_to_above_{i}",
            precondition = Transition("pre", 
                                    g(f"!robot_run && robot_state == initial && robot_pose != above_{i} && robot_pose == {i}"), 
                                    a(f"robot_command = move_j, robot_run, robot_goal_frame = above_{i}")),

            postcondition = Transition("post", g(f"robot_state == done"), 
                                        a(f"!robot_run, robot_pose <- above_{i}, above_position")),
    ...
    for i in ["red", "green", "blue"]:
        for j in ["buffer", "pose_1", "pose_2","pose_3"]:
            ops[f"pick_{i}_at_{j}"] = Operation(
                name = f"pick_{i}_at_{j}",

                precondition = Transition("pre",
                                          g(f"above_position == True && {i}_cube_at == {j} && !gripper_run && robot_pose == {j} && !robot_run && !gripper_closed"),
                                          a(f"gripper_command = pick_{i}, gripper_run, robot_run, robot_command = move_j, gripper_closed")),
                postcondition = Transition("post",
                                           g(f"robot_state == done"),
                                           a(f"!gripper_run, {i}_cube_at <- gripper, !robot_run, !above_position ")), #robot_pose <- above_{j}
                effects = (),
                to_run = Transition.default()
            )
    #place
    for i in ["red", "green", "blue"]:
        for j in ["buffer", "pose_1", "pose_2","pose_3"]:
            ops[f"place_{i}_at_{j}"] = Operation(
                name = f"place_{i}_at_{j}",

                precondition = Transition("pre",
                                          g(f"above_position == True && {i}_cube_at == gripper && !gripper_run && robot_pose == {j} && !robot_run && gripper_closed && red_cube_at != {j} && blue_cube_at != {j} && green_cube_at != {j}"),
                                          a(f"gripper_command <- place_{i}, gripper_run, robot_run, robot_command = move_j, !gripper_closed")),
                postcondition = Transition("post",
                                           g(f"robot_state == done"),
                                           a(f"!gripper_run, {i}_cube_at <- {j}, gripper_command <- none, !robot_run, !above_position ")), 
                effects = (),
                to_run = Transition.default()
            )
