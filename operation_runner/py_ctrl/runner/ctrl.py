...
def tick_the_runner(state: State, model: Model, pre_start: bool) -> State:
    ...
    #what this does is that it look at what free transitions can be done - We are going to do something similar to this with operations
    """THIS IS EXACTLY WHAT WE ARE GOING TO DO FOR THE OPERATIONS!!!!!!!!!!!!!!"""
    ...
    if not the_plan:
        return state.next(plan_status="No plan in state", runner_plan = None, step_in_plan = None) #there is no plan
    ...
    ###TODO Rewrite these lines of code (make a function that is called simple operation runner)
