...
class CompetitionInterface(Node):
    ...
    def __init__(self, enable_moveit=True):
        ...
        # Moveit_py variables
        self.moveit_enabled = False #enable_moveit
        ...
        ###My bullshit
        self.my_camera_sub = self.create_subscription(ImageMsg,
                                                                  "/ariac/sensors/my_camera/rgb_image",
                                                                  self._my_camera_cb,
                                                                  qos_profile_sensor_data,
                                                                  # callback group,
                                                                  )
        self.my_other_camera_sub = self.create_subscription(AdvancedLogicalCameraImageMsg,
        ...
        """#date of writing this code attempt 15/3
        #publish the my_other_camera output as a topic so we can listen to it from another node
        self.publisher_my_other_camera_ = self.create_publisher(String, 'topic', 10)#Declares that the node publishes messages of type String (imported from the std_msgs.msg module)
        ...
        timer_period = 1  # seconds #execute every 0.5 seconds
        self.timer_my_other_camera = self.create_timer(timer_period, self.timer_callback_my_other_camera) #timer_callback creates a message with the counter value appended, and publishes it to the console with get_logger().info.
       
        self.i = 0 #counter used in the callback
        
    def timer_callback_my_other_camera(self):
        msg = String()
        msg.data = " info part1:%s, info part 2: %s, msg nr:  %d" % (str(self._my_other_camera_parts), str(self._my_other_camera_pose), self.i) #%d operator allows you to add numbers within the strings
        self.publisher_.publish(msg)
        self.get_logger().info('Publishing: "%d"' % msg.data) #%s is a place holder for a string in a formating operation
        self.i += 1
    """
    ...
    def _my_camera_cb(self, msg: ImageMsg):
        try:
            self._my_camera_image = self._bridge.imgmsg_to_cv2(msg, "bgr8")
        except CvBridgeError as e:
            print(e)
    ...
    def _my_other_camera_cb(self,msg : AdvancedLogicalCameraImageMsg):
        self._my_other_camera_parts = msg.part_poses
        self._my_other_camera_pose = msg.sensor_pose
        
        """#I just want to try to save some information
        i = 0
        newState ='{"info" :[{"parts": %s}, {"pose": %s}]}' %(str(self._my_other_camera_parts), str(self._my_other_camera_pose)) #writing in a json format

        with open(f"images/info{i}.json", 'w', encoding='utf-8') as f:
            json.dump(newState, f, ensure_ascii=False, indent=4)
        i += 1
        f.close()
        self.get_logger().info('My_other_camera: part: %s, pose %s' % ( str(self._my_other_camera_parts), str(self._my_other_camera_pose)))"""
