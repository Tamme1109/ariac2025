...
"""
    ...
    See this link to remeber what the methods do: https://docs.python.org/3/reference/datamodel.html#object.__lt__ 
    ...
    (i.e. store the value in the instance dictionary). name is the attribute name, value is the value to be
    assigned to it. If __setattr__() wants to assign to an instance attribute, it should call the base class method
    with the same name, for example, object.__setattr__(self, name, value).
 
    object.__delattr__(self, name), Like __setattr__() but for attribute deletion instead of assignment. 
...
class Eq:
    ...
    def eval(self, state: State) -> bool:
        ...
        If the variable is not included in the state a NotInStateException will be raised
        """
        x = state.get(self.variable) #first element for comparison, e.g v1 == "foo"
        y = self.value_or_variable #second element for comparison, e.g v2
        if self.value_or_variable in state.state: #look for e.g. v2 in the state
            y = state.get(self.value_or_variable) #look at the state value, e.g. v2 == True
...
class And:
    ...
    def eval(self, state) -> bool:
        ...
        return all(res) #returns true if all values in res is true, false otherwise
...
class Or:
    ...
    def eval(self, state) -> bool:
        ...
        """
        res = [g.eval(state) for g in self.guards]
        return any(res) #returns tru if any element in res is true, false otherwise
